...
import { IRequestBody } from "@interfaces/auth/loginGoogle.interface";
...
const loginGoogleController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await db
      ...
      .andWhere("data.googleId = :googleId", { googleId: body.google_id })
    ...
    errorLogger.error(
      `${error.status || 500} - [src/controllers/auth/loginGoogle.controller.ts] - ${error.message} - ${req.originalUrl} - ${req.method} - ${
        req.ip
      } - ${parseUserAgent(req)}`
    );
...
export default loginGoogleController;
