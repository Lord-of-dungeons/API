...
import { IRequestBody } from "@interfaces/auth/registerGithub.interface";
...
const registerGithubController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const password = await Password.hash(body.github_id?.toString());
    ...
    user.profilePicturePath = body.profile_picture_path;
    user.role = "USER";
    user.githubId = body.github_id?.toString();
    ...
    errorLogger.error(
      `${error.status || 500} - [src/controllers/auth/registerGithub.controller.ts] - ${error.message} - ${req.originalUrl} - ${req.method} - ${
        req.ip
      } - ${parseUserAgent(req)}`
    );

    res.status(500).json({ message: "Erreur Serveur survenue lors de l'inscription avec Github. Veuillez réessayer plus tard" });
  }
};

export default registerGithubController;
