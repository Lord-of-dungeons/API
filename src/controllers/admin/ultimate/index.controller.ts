...
import { IRequestBody as IRequestBodyAdd } from "@interfaces/admin/ultimate/add.interface";
import { IRequestBody as IRequestBodyUpdate } from "@interfaces/admin/ultimate/update.interface";
 ...
 *  @param {Response} res
 */
export const deleteUltimateController = async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
    return res.status(200).json({ message: "OK. Veuillez réessayer plus tard" });
  } catch (error) {
